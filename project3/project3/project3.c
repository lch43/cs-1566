...
int createBall(vec4 * vertices, vec2 * tex_coords, vec4 * colors, vec4 * normals, ball * ball, settings settings, int vertOffset, int isLight)
{
    ...
    for(i=startOffset; i<vertOffset;i++)
    {
        ...
        //Set topLeft and bottomRight to the texture position for each different ball.
        ...
        //Determine the position on the texture for the vertex
        ...
        //If the ball is a light source then make it white.
        if (isLight == 1)
        {
...
//This function just loops through the 16 balls and add them into the poolBalls array.
...
void init(void)
{
    ...
    //Put the universal settings of the project into a struct to pass through wherever needed
    settings Settings;
    ...
    //Quick math for the amount of vertices.
    ...
    //Set the number of vertices and create the arrays for rendering.
    num_vertices = tableVertices + verticesPerBall * Settings.numBalls + verticesPerBall;
    vec4 vertices[num_vertices]; //To give position
    vec4 colors[num_vertices]; //To color non-textured objects
    vec4 normals[num_vertices]; //To help with light
    vec2 tex_coords[num_vertices]; //To texture objects
    ...
    //Set the normals for the table (this could have been done earlier, but it was added later when light was added)
    int i=0;
...
//This enum isnt really used. I was a bit more hopeful of being neat... guess not. I could have just used an int.
typedef enum
{
    LINE_UP = 0,
    ORBIT,
...
int ballLineNum = 0; //Which ball is moving into line
vec4 ballOriginalSpot; //Where the ball was before moving to the line
int tempMax = -1; //Max steps in the animation. This is dynamic and changes based on distance from ball to target.
    ...
    if (isAnimating)
    {
        ...
        if (currentState == 0) //LINE_UP
        {
            if (ballLineNum <= 16)
            {
                ...
                {
                    //Set up next ball in line
                    currentStep = 0;
                    ballLineNum++;
                    ball = ballLineNum%16; //Start at ball 1 then move on to ball 15 then ball 0
                ...
                {
                    float alpha = (currentStep*1.00/tempMax); //Find how far along the animation we are.
                    vec4 goal;
                    if (ball != 0)
                    {
                        goal = (vec4){0.0,0,(-3.0+((ball-1)*.2)),1.0}; //Set goal for balls 1-15
                    }
                    else
                    {
                        goal = (vec4){0.0,0,0.0,1.0}; //Set goal for cue ball
                    }
                    ...
                    //Place ball in final spot for frame.
        ...
        {
            //Move balls around the cue ball. The further out the slower the movement.
            int i=15;
            ...
            //If the camera is in follow mode, then set the eye and at.
            if (camMode >= 0)
            {
                i = camMode;
                float radius = -3+.2*(i-1);
...
{
    /*Landon Higinbotham's code starts here*/
    if (key == ' ' && currentState == 0 && isAnimating == 0) //Animate on space press.
    {
        ballLineNum = 0;
        currentState = 0;
        currentStep = 0;
    ...
    if (key == 'w') //Move eye up on glass ball.
    {
        lookEye = moveEye(0,-5,0);
        glutPostRedisplay();
    }
    if (key == 's') //Move eye down on glass ball.
    {
        lookEye = moveEye(0,5,0);
        glutPostRedisplay();
    }
    if (key == 'a') //Move eye left on glass ball.
    {
        lookEye = moveEye(5,0,0);
        glutPostRedisplay();
    }
    if (key == 'd') //Move eye right on glass ball.
    {
        lookEye = moveEye(-5,0,0);
        glutPostRedisplay();
    }
    if (key == 'e') //Move eye in on glass ball.
    {
        lookEye = moveEye(0,0,-.05);
        glutPostRedisplay();
    }
    if (key == 'q') //Move eye out on glass ball.
    {
        lookEye = moveEye(0,0,.05);
        glutPostRedisplay();
    }
    if (key >= 49 && key <= 57) //Follow balls 1-9
    {
        camMode = key-48;
        glutPostRedisplay();
    }
    if (key  == 'r') //Follow ball 10
    {
        camMode = 10;
        glutPostRedisplay();
    }
    if (key  == 't') //Follow ball 11
    {
        camMode = 11;
        glutPostRedisplay();
    }
    if (key  == 'y') //Follow ball 12
    {
        camMode = 12;
        glutPostRedisplay();
    }
    if (key  == 'f') //Follow ball 13
    {
        camMode = 13;
        glutPostRedisplay();
    }
    if (key  == 'g') //Follow ball 14
    {
        camMode = 14;
        glutPostRedisplay();
    }
    if (key  == 'h') //Follow ball 15
    {
        camMode = 15;
        glutPostRedisplay();
    }
    if (key == '0') //Rest back to the normal view
    {
        camMode = -1;
        lookEye = startEye;
        lookAt = startAt;
